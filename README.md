...
> **🌟 Powered by MeshAPI**
> These tutorials are optimized to run using **[MeshAPI](https://meshapi.ai)**, a powerful and flexible platform that provides seamless access to top-tier models (like GPT-4 and Claude) with a unified endpoint. Using MeshAPI allows you to switch models dynamically and securely without changing your core application code!
...
# 3. Configure API key
cp .env.example .env
# Edit .env and add your MeshAPI Key (OPENAI_API_KEY=sk-your-meshapi-key) and OPENAI_BASE_URL=https://api.meshapi.ai/v1
...
1. Sign up or log in at MeshAPI platform
...
# OPENAI_API_KEY=sk-your-meshapi-key-here
...
These tutorials use the **MeshAPI** service instead of direct OpenAI endpoints. 
Here's what to expect:

- **Notebooks are configured dynamically**: All notebooks have been updated to use `os.getenv("OPENAI_MODEL", "openai/gpt-5.4")`. This means you can easily switch models by updating your `.env` file!
- **Default configuration**: The default model is set to `openai/gpt-5.4` via MeshAPI.
- **Cost**: Refer to the MeshAPI pricing for your chosen model.

Tips to minimize costs:
- Start with a lighter model if available via MeshAPI for practice
- Set usage limits in your MeshAPI account
